  ...
  function showTip(evt, item, value, pinned = false) {
    ...
    if (stat && stat.spansDisruption) {
      ...
      parts.push(
        ...
        /* n counts OVERLAPPING windows, and consecutive ones share all but
           one month of their span, so a big n at a long horizon is far weaker
           evidence than the same n at one month: 305 three-year windows are
           about 8 independent observations, while 212 one-month changes are
           212. Showing only n inverted the reader's sense of which score to
           trust, so both are reported whenever they differ. */
        `(n=${stat.n}` +
        (stat.independent < stat.n - 0.5
          ? `, about ${Math.round(stat.independent)} independent`
          : "") +
        `, pandemic windows excluded)</span>` +
